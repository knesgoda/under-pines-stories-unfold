...
export default function MobileTabBar() {
  ...
  const Item = ({ href, Icon }: { href: string; Icon: ComponentType<SVGProps<SVGSVGElement>> }) => {
    ...
    return (
      <Link to={href} className={clsx('flex-1 grid place-items-center h-12', active ? 'text-white' : 'text-white/70')}>
        <Icon />
  ...
  return (
    <div className="lg:hidden">
      <div className="fixed bottom-0 inset-x-0 border-t border-white/10 bg-[#0B1C13]/95 backdrop-blur z-40">
        <div className="relative flex items-center">
          ...
          <div className="relative -mt-6 flex-1 grid place-items-center">
            <Link to="/compose" className="grid place-items-center h-12 w-12 rounded-full bg-background-sand text-black shadow-md">
              <PlusIcon />
      ...
      <Link to="/embers" className="fixed right-4 bottom-20 grid place-items-center h-10 w-10 rounded-full bg-white/10 text-white/90 backdrop-blur z-40">
        <SparkIcon />
